...
const astHelpers = {
  containsStyleSheetObject: function(node, objectNames) {
    ...
    //   node.params[0].name === "theme"
    // ) {
    //   console.log("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
    //   console.log(node.body.properties);
    //   console.log("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
    // }
  ...
  isStyleSheetDeclaration: function(node, settings) {
    ...
    return astHelpers.containsStyleSheetObject(node, objectNames);
  },
  ...
  getStyleDeclarations: function(node) {
    if (node && node.body && node.body.properties) {
      return node.body.properties.filter(
  ...
  getPotentialStyleReferenceFromMemberExpression: function(node) {
    if (
      ...
      return ["styles", node.property.name].join(".");
    }
  },
